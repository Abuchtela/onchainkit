...
import { MiniKitContext, type MiniKitContextType, MiniKitProvider } from './MiniKitProvider';
...
describe('MiniKitProvider', () => {
    ...
    render(
      <WagmiProvider config={createConfig(mockConfig)}>
        <QueryClientProvider client={queryClient}>
          <MiniKitProvider chain={mockConfig.chains[0]} config={{
            appearance: {
              name: 'Test App',
  ...
  it('should set up frame event listeners', async () => {
    ...
    expect(sdk.on).toHaveBeenCalledWith('notificationsEnabled', expect.any(Function));
    expect(sdk.on).toHaveBeenCalledWith('notificationsDisabled', expect.any(Function));
  });
  ...
  it('should update context when frame is added and removed', async () => {
    ...
    expect(contextValue?.context?.client.notificationDetails).toEqual(notificationDetails);
  ...
  it('should log an error when frameAddRejected is emitted', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Frame add rejected', 'invalid_domain_manifest');
  });
  ...
  it('should handle context fetch error', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching context:', expect.any(Error));
    expect(contextValue?.context).toBeNull();
